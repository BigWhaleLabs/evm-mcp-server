import {
  McpServer,
  ResourceTemplate,
} from '@big-whale-labs/modelcontextprotocol-sdk/server/mcp.js'
...
export function registerEVMResources(server: McpServer) {
  ...
  server.resource('ethereum_chain_info', 'evm://chain', async (uri) => {
    try {
  ...
  server.resource(
    'default_eth_balance',
    new ResourceTemplate('evm://address/{address}/eth-balance', {
      list: undefined,
    }),
    async (uri, params) => {
      try {
